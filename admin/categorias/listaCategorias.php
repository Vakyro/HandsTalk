<?php
  ...
  $Tabla = "categorias";
  $Id = "IdCategoria";
  $Accion = "verlistaCategorias";
?>

<div class="mb-4 text-2xl leadi pl-56 font-bold text-[#8fb2fc]">
  <h2 class="flex items-center">
    <span class="text-5xl mr-8">Categorias</span>
    <a href="<?=ROOTURL?>?accion=formCategoria" class="inline-block pt-2">
...
<?php foreach($listaCategorias as $renglon => $campo) { ?>
        <tr class="bg-white border-b font-bold text-[#8fb2fc] hover:text-[#5077ca] transition-colors duration-700 cursor-default text-center">			
          ...
          <td class="px-6 py-4 text-right text-" colspan="2">
            <a href="<?=ROOTURL?>?accion=formEditCategoria&IdCategoria=<?=$campo['IdCategoria']?>" class=" text-[#8fb2fc] hover:text-[#df8181] transition-colors duration-700">
